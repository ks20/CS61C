...
class YelpClassifier(object):
    ...
    def review_to_word_counts(review):
        num_stars = review[1]
        review_text_as_string = review[2]
        concatList = []
        splitList = review_text_as_string.split(" ")
        for word in splitList:
            key = (num_stars, str(word))
            concatList = concatList + [(key, 1)]
        return concatList
    ...
    def counts_to_probabilities(self, num_stars_and_word_counts):
        stars = num_stars_and_word_counts[0][0]
        word = num_stars_and_word_counts[0][1]
        count_of_word = num_stars_and_word_counts[1]
        map_dict = {}
        #print(stars)
        likelihood = self.compute_likelihood(count_of_word, int(stars))
        map_dict[str(word)] = likelihood
        return (stars, map_dict)
    ...
    def review_to_num_stars_num_words(review):
        ...
        splitString = review_text.split(" ")
        num_of_words_in_review = len(splitString)
        #return (num_stars, (num_of_words_in_review, 1))
        return (num_stars, (1, num_of_words_in_review))
    ...
    def add_review_and_word_counts(count1, count2):
        value_of_pair_1 = count1[0] + count2[0]
        value_of_pair_2 = count1[1] + count2[1]
        return (value_of_pair_1, value_of_pair_2)
